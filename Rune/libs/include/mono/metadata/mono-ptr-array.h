...
#define mono_ptr_array_init(ARRAY, INITIAL_SIZE, SOURCE, KEY, MSG)                                                                \
    do                                                                                                                            \
    {                                                                                                                             \
        (ARRAY).size = 0;                                                                                                         \
        ...
        (ARRAY).data = INITIAL_SIZE > MONO_PTR_ARRAY_MAX_ON_STACK                                                                 \
                           ? (void**)mono_gc_alloc_fixed(                                                                         \
                                 sizeof(void*) * INITIAL_SIZE, mono_gc_make_root_descr_all_refs(INITIAL_SIZE), SOURCE, NULL, MSG) \
                           : g_newa(void*, MONO_PTR_ARRAY_MAX_ON_STACK);                                                          \
    } while (0)

#define mono_ptr_array_destroy(ARRAY)                       \
    do                                                      \
    {                                                       \
...
#define mono_ptr_array_append(ARRAY, VALUE)                                                                    \
    do                                                                                                         \
    {                                                                                                          \
        if ((ARRAY).size >= (ARRAY).capacity)                                                                  \
        {                                                                                                      \
            void** __tmp = (void**)mono_gc_alloc_fixed(sizeof(void*) * (ARRAY).capacity * 2,                   \
                                                       mono_gc_make_root_descr_all_refs((ARRAY).capacity * 2), \
                                                       (ARRAY).source,                                         \
                                                       (ARRAY).key,                                            \
                                                       (ARRAY).msg);                                           \
...
#define mono_ptr_array_sort(ARRAY, COMPARE_FUNC)                                  \
    do                                                                            \
    {                                                                             \
        mono_qsort((ARRAY).data, (ARRAY).size, sizeof(gpointer), (COMPARE_FUNC)); \
    } while (0)

#define mono_ptr_array_set(ARRAY, IDX, VALUE) \
    do                                        \
    {                                         \
...
#define mono_ptr_array_reset(ARRAY) \
    do                              \
    {                               \
        (ARRAY).size = 0;           \
    } while (0)

#define mono_ptr_array_clear(ARRAY)                                            \
    do                                                                         \
    {                                                                          \
        (ARRAY).size = 0;                                                      \
