...
#define MONO_GC_REGISTER_ROOT_PINNING(x, src, key, msg) \
    mono_gc_register_root((char*)&(x), sizeof(x), MONO_GC_DESCRIPTOR_NULL, (src), (key), (msg))
...
#define MONO_GC_REGISTER_ROOT_SINGLE(x, src, key, msg)                                                                     \
    do                                                                                                                     \
    {                                                                                                                      \
 ...
 * when using Boehm.
 */
#define MONO_GC_REGISTER_ROOT_IF_MOVING(x, src, key, msg)   \
    do                                                      \
    {                                                       \
...
void mono_object_register_finalizer_handle(MonoObjectHandle obj);
...
MonoObject* mono_gc_alloc_fixed_no_descriptor(size_t size, MonoGCRootSource source, void* key, const char* msg);
...
MonoObjectHandle mono_gc_alloc_handle_pinned_obj(MonoVTable* vtable, gsize size);

MonoObject* mono_gc_alloc_obj(MonoVTable* vtable, size_t size);

MonoObjectHandle mono_gc_alloc_handle_obj(MonoVTable* vtable, gsize size);

MonoArray* mono_gc_alloc_vector(MonoVTable* vtable, size_t size, uintptr_t max_length);

MonoArrayHandle mono_gc_alloc_handle_vector(MonoVTable* vtable, gsize size, gsize max_length);

MonoArray* mono_gc_alloc_array(MonoVTable* vtable, size_t size, uintptr_t max_length, uintptr_t bounds_size);

MonoArrayHandle mono_gc_alloc_handle_array(MonoVTable* vtable, gsize size, gsize max_length, gsize bounds_size);

MonoString* mono_gc_alloc_string(MonoVTable* vtable, size_t size, gint32 len);

MonoStringHandle mono_gc_alloc_handle_string(MonoVTable* vtable, gsize size, gint32 len);
...
MonoObjectHandle mono_gc_alloc_handle_mature(MonoVTable* vtable, gsize size);

void mono_gc_register_obj_with_weak_fields(void* obj);
void mono_gc_register_object_with_weak_fields(MonoObjectHandle obj);
...
#define MONO_ROOT_SETREF(s, fieldname, value)                             \
    do                                                                    \
    {                                                                     \
...
MonoRangeCopyFunction mono_gc_get_range_copy_func(void);
...
G_EXTERN_C  // due to THREAD_INFO_TYPE varying
    void
    mono_gc_thread_detach_with_lock(THREAD_INFO_TYPE* info);
