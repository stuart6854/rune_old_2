...
static inline void mono_stack_mark_init(MonoThreadInfo* info, HandleStackMark* stackmark)
{
#ifdef MONO_HANDLE_TRACK_SP
...
static inline void mono_stack_mark_pop(MonoThreadInfo* info, HandleStackMark* stackmark)
{
    HandleStack* handles = info->handle_stack;
...
#define CLEAR_ICALL_COMMON mono_error_set_pending_exception(error);
...
#define CLEAR_ICALL_FRAME mono_stack_mark_pop(mono_stack_mark_record_size(mono_thread_info_current_var, &__mark, __FUNCTION__), &__mark);

// FIXME This should be one function call since it is not fully inlined.
#define CLEAR_ICALL_FRAME_VALUE(RESULT, HANDLE) \
    (RESULT) = g_cast(                          \
        mono_stack_mark_pop_value(mono_stack_mark_record_size(mono_thread_info_current_var, &__mark, __FUNCTION__), &__mark, (HANDLE)));

#define HANDLE_FUNCTION_ENTER()                                                               \
    do                                                                                        \
    {                                                                                         \
        MONO_DISABLE_WARNING(4459) /* declaration of 'identifier' hides global declaration */ \
        /* There are deliberately locals and a constant NULL global with this same name. */   \
...
#define SETUP_ICALL_FUNCTION                                                              \
    MONO_DISABLE_WARNING(4459) /* declaration of 'identifier' hides global declaration */ \
    /* There are deliberately locals and a constant NULL global with this same name. */   \
    MonoThreadInfo* mono_thread_info_current_var = mono_thread_info_current() MONO_RESTORE_WARNING
...
static void mono_thread_info_pop_stack_mark(MonoThreadInfo* info, void* old_mark)
{
    info->stack_mark = old_mark;
}

static void* mono_thread_info_push_stack_mark(MonoThreadInfo* info, void* mark)
{
    ...
    #define CLEAR_STACK_WATERMARK mono_thread_info_pop_stack_mark(mono_thread_info_current_var, __old_stack_mark);
...
*/
#define MONO_HANDLE_SETRAW(HANDLE, FIELD, VALUE)                                                    \
    do                                                                                              \
    {                                                                                               \
...
#define MONO_HANDLE_SET(HANDLE, FIELD, VALUE)                                                                            \
    do                                                                                                                   \
    {                                                                                                                    \
...
#define MONO_HANDLE_GET(RESULT, HANDLE, FIELD)                                                                             \
    do                                                                                                                     \
    {                                                                                                                      \
...
#define MONO_HANDLE_NEW_GET(TYPE, HANDLE, FIELD) \
    (MONO_HANDLE_NEW(TYPE, MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(MONO_HANDLE_UNSUPPRESS(HANDLE))->FIELD)))
...
#define MONO_HANDLE_SETVAL(HANDLE, FIELD, TYPE, VALUE)                                             \
    do                                                                                             \
    {                                                                                              \
        TYPE __val = (VALUE);                                                                      \
        if (0)                                                                                     \
        {                                                                                          \
            TYPE* typecheck G_GNUC_UNUSED = &MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(HANDLE)->FIELD); \
        }                                                                                          \
        MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(MONO_HANDLE_UNSUPPRESS(HANDLE))->FIELD = __val);      \
    } while (0)

// handle [idx] = value (for managed pointers)
#define MONO_HANDLE_ARRAY_SETREF(HANDLE, IDX, VALUE)                                                                \
    do                                                                                                              \
    {                                                                                                               \
        uintptr_t __idx = (IDX);                                                                                    \
        MonoObjectHandle __val = MONO_HANDLE_CAST(MonoObject, VALUE);                                               \
...
#define MONO_HANDLE_ARRAY_SETVAL(HANDLE, TYPE, IDX, VALUE)                                                \
    do                                                                                                    \
    {                                                                                                     \
        uintptr_t __idx = (IDX);                                                                          \
        TYPE __val = (VALUE);                                                                             \
...
// handle [idx] = value
    #define MONO_HANDLE_ARRAY_SETRAW(HANDLE, IDX, VALUE)                                           \
        do                                                                                         \
        {                                                                                          \
            MONO_HANDLE_SUPPRESS_SCOPE(1);                                                         \
...
#define MONO_HANDLE_ARRAY_GETVAL(DEST, HANDLE, TYPE, IDX)                                                   \
    do                                                                                                      \
    {                                                                                                       \
...
#define MONO_HANDLE_DOMAIN(HANDLE) \
    MONO_HANDLE_SUPPRESS(mono_object_domain(MONO_HANDLE_RAW(MONO_HANDLE_CAST(MonoObject, MONO_HANDLE_UNSUPPRESS(HANDLE)))))
...
#define MONO_HANDLE_GET_FIELD_VAL(HANDLE, TYPE, FIELD) \
    (*(TYPE*)(mono_handle_unsafe_field_addr(MONO_HANDLE_CAST(MonoObject, (HANDLE)), (FIELD))))
#define MONO_HANDLE_GET_FIELD_BOOL(handle, type, field) (MONO_BOOL(MONO_HANDLE_GET_FIELD_VAL((handle), type, (field))))

#define MONO_HANDLE_NEW_GET_FIELD(HANDLE, TYPE, FIELD) \
    MONO_HANDLE_NEW(TYPE,                              \
                    MONO_HANDLE_SUPPRESS(              \
                        *(TYPE**)(mono_handle_unsafe_field_addr(MONO_HANDLE_CAST(MonoObject, MONO_HANDLE_UNSUPPRESS(HANDLE)), (FIELD)))))

#define MONO_HANDLE_SET_FIELD_VAL(HANDLE, TYPE, FIELD, VAL)                \
    do                                                                     \
    {                                                                      \
...
#define MONO_HANDLE_SET_FIELD_REF(HANDLE, FIELD, VALH)                                                                         \
    do                                                                                                                         \
    {                                                                                                                          \
        ...
        MONO_HANDLE_SUPPRESS(                                                                                                  \
            mono_gc_wbarrier_generic_store_internal(mono_handle_unsafe_field_addr(__obj, __field), MONO_HANDLE_RAW(__value))); \
    } while (0)
...
static inline MonoObjectHandle mono_handle_cast(gpointer a)
{
    return *(MonoObjectHandle*)&a;
}

static inline MONO_ALWAYS_INLINE gboolean mono_handle_is_null(MonoRawHandle raw_handle)
{
    MONO_HANDLE_SUPPRESS_SCOPE(1);
...
static inline MONO_ALWAYS_INLINE gpointer mono_handle_raw(MonoRawHandle raw_handle)
{
    MONO_HANDLE_SUPPRESS_SCOPE(1);
    MonoObjectHandle* handle = (MonoObjectHandle*)&raw_handle;
...
#define NULL_HANDLE_INIT \
    {                    \
        0                \
    }
static inline MonoObjectHandle mono_null_value_handle(void)
{
...
static inline void volatile* mono_handle_ref(void volatile* p)
{
    g_assert(p);
    return p;
}
...
static inline MonoObjectHandle mono_handle_assign_raw(MonoObjectHandleOut dest, void* src)
{
    g_assert(dest.__raw);
...
static inline gpointer mono_handle_unsafe_field_addr(MonoObjectHandle h, MonoClassField* field)
{
...
MonoArrayHandle mono_array_new_full_handle(
    MonoDomain* domain, MonoClass* array_class, uintptr_t* lengths, intptr_t* lower_bounds, MonoError* error);

#define mono_array_handle_setref(array, index, value) MONO_HANDLE_ARRAY_SETREF((array), (index), (value))

void mono_handle_array_getref(MonoObjectHandleOut dest, MonoArrayHandle array, uintptr_t index);
...
uint32_t mono_gchandle_from_handle(MonoObjectHandle handle, mono_bool pinned);

MonoObjectHandle mono_gchandle_get_target_handle(uint32_t gchandle);

gboolean mono_gchandle_target_equal(uint32_t gchandle, MonoObjectHandle equal);

void mono_gchandle_target_is_null_or_equal(uint32_t gchandle, MonoObjectHandle equal, gboolean* is_null, gboolean* is_equal);

void mono_gchandle_set_target_handle(guint32 gchandle, MonoObjectHandle obj);

void mono_array_handle_memcpy_refs(MonoArrayHandle dest, uintptr_t dest_idx, MonoArrayHandle src, uintptr_t src_idx, uintptr_t len);
 ...
 */
gpointer mono_array_handle_pin_with_size(MonoArrayHandle handle, int size, uintptr_t index, uint32_t* gchandle);
...
void mono_value_copy_array_handle(MonoArrayHandle dest, int dest_idx, gconstpointer src, int count);

gunichar2* mono_string_handle_pin_chars(MonoStringHandle s, uint32_t* gchandle_out);

gpointer mono_object_handle_pin_unbox(MonoObjectHandle boxed_valuetype_obj, uint32_t* gchandle_out);

static inline gpointer mono_handle_unbox_unsafe(MonoObjectHandle handle)
{
    g_assert(m_class_is_valuetype(MONO_HANDLE_GETVAL(handle, vtable)->klass));
    return MONO_HANDLE_SUPPRESS(MONO_HANDLE_RAW(handle) + 1);
}

void mono_error_set_exception_handle(MonoError* error, MonoExceptionHandle exc);

MonoAppContextHandle mono_context_get_handle(void);

void mono_context_set_handle(MonoAppContextHandle new_context);

guint32 mono_gchandle_new_weakref_from_handle(MonoObjectHandle handle);

int mono_handle_hash(MonoObjectHandle object);

guint32 mono_gchandle_new_weakref_from_handle_track_resurrection(MonoObjectHandle handle);
