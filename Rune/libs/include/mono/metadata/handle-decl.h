...
#define TYPED_HANDLE_DECL(TYPE)                                                                               \
    ...
    {                                                                                                         \
        MONO_IF_CPLUSPLUS(                                                                                    \
            MONO_ALWAYS_INLINE TYPE* GetRaw() const { return __raw ? *__raw : NULL; } TYPE* volatile* Ref() { \
                g_assert(__raw);                                                                              \
                return __raw;                                                                                 \
            })                                                                                                \
        TYPE* volatile* __raw;                                                                                \
    } TYPED_HANDLE_NAME(TYPE), TYPED_OUT_HANDLE_NAME(TYPE), TYPED_IN_OUT_HANDLE_NAME(TYPE);                   \
    ...
    static inline MONO_ALWAYS_INLINE TYPED_HANDLE_NAME(TYPE) MONO_HANDLE_CAST_FOR(TYPE)(MonoRawHandle a)      \
    {                                                                                                         \
        TYPED_HANDLE_NAME(TYPE) b = { (TYPE**)a };                                                            \
        return b;                                                                                             \
    }                                                                                                         \
    static inline MONO_ALWAYS_INLINE MonoObject* MONO_HANDLE_TYPECHECK_FOR(TYPE)(TYPE * a)                    \
    {                                                                                                         \
        return (MonoObject*)a;                                                                                \
    }                                                                                                         \
    /* Out/InOut synonyms for icall-def.h HANDLES () */                                                       \
    static inline MONO_ALWAYS_INLINE TYPED_HANDLE_NAME(TYPE) MONO_HANDLE_CAST_FOR(TYPE##Out)(gpointer a)      \
    {                                                                                                         \
        return MONO_HANDLE_CAST_FOR(TYPE)(a);                                                                 \
    }                                                                                                         \
    static inline MONO_ALWAYS_INLINE MonoObject* MONO_HANDLE_TYPECHECK_FOR(TYPE##Out)(TYPE * a)               \
    {                                                                                                         \
        return MONO_HANDLE_TYPECHECK_FOR(TYPE)(a);                                                            \
    }                                                                                                         \
    static inline MONO_ALWAYS_INLINE TYPED_HANDLE_NAME(TYPE) MONO_HANDLE_CAST_FOR(TYPE##InOut)(gpointer a)    \
    {                                                                                                         \
        return MONO_HANDLE_CAST_FOR(TYPE)(a);                                                                 \
    }                                                                                                         \
    static inline MONO_ALWAYS_INLINE MonoObject* MONO_HANDLE_TYPECHECK_FOR(TYPE##InOut)(TYPE * a)             \
    {                                                                                                         \
        return MONO_HANDLE_TYPECHECK_FOR(TYPE)(a);                                                            \
    }
