...
namespace entt
{
    ...
    namespace internal
    {
        ...
        struct extended_view_iterator final
        {
            ...
            extended_view_iterator(It from, std::tuple<Type*...> value) : it{ from }, pools{ value } {}
            ...
            {
                return std::apply(
                    [entt = *it](auto*... curr) { return std::tuple_cat(std::make_tuple(entt), curr->get_as_tuple(entt)...); }, pools);
            }
    ...
    class basic_view<get_t<Get...>, exclude_t<Exclude...>>
    {
        ...
        void unchecked_refresh() noexcept
        {
            ...
            std::apply([this](auto*, auto*... other) { ((this->view = other->size() < this->view->size() ? other : this->view), ...); },
                       pools);
        }
    ...
    public:
         ...
         */
        basic_view(Get&... value, Exclude&... excl) noexcept : pools{ &value... }, filter{ &excl... }, view{} { unchecked_refresh(); }
         ...
         */
        [[nodiscard]] size_type size_hint() const noexcept { return view ? view->size() : size_type{}; }
         ...
         */
        [[nodiscard]] decltype(auto) operator[](const entity_type entt) const { return get(entt); }
        ...
        {
            return view && std::apply([entt](const auto*... curr) { return (curr->contains(entt) && ...); }, pools) &&
                   internal::none_of(filter, entt);
        }
    ...
    public:
         ...
         */
        basic_view(Get& value) noexcept : pools{ &value }, filter{}, view{ &value } {}

        /**
         * @brief Constructs a single-type view from a storage class.
         * @param value The storage for the type to iterate.
         */
        basic_view(std::tuple<Get&> value, std::tuple<> = {}) noexcept : basic_view{ std::get<0>(value) } {}
         ...
         * @param elem A storage to assign to the view.
         */
        void storage(Get& elem) noexcept { storage<0>(elem); }
         ...
         */
        [[nodiscard]] size_type size() const noexcept { return view ? view->size() : size_type{}; }

        /**
         * @brief Checks whether a view is empty.
         * @return True if the view is empty, false otherwise.
         */
        [[nodiscard]] bool empty() const noexcept { return !view || view->empty(); }
         ...
         * @return An iterator to the first entity of the view.
         */
        [[nodiscard]] iterator begin() const noexcept { return view ? view->begin() : iterator{}; }

        /**
         * @brief Returns an iterator that is past the last entity of the view.
         * @return An iterator to the entity following the last entity of the view.
         */
        [[nodiscard]] iterator end() const noexcept { return view ? view->end() : iterator{}; }
         ...
         */
        [[nodiscard]] reverse_iterator rbegin() const noexcept { return view ? view->rbegin() : reverse_iterator{}; }
         ...
         * reversed view.
         */
        [[nodiscard]] reverse_iterator rend() const noexcept { return view ? view->rend() : reverse_iterator{}; }
         ...
         * otherwise.
         */
        [[nodiscard]] entity_type front() const noexcept { return (!view || view->empty()) ? null : *view->begin(); }
         ...
         * otherwise.
         */
        [[nodiscard]] entity_type back() const noexcept { return (!view || view->empty()) ? null : *view->rbegin(); }
         ...
         * iterator otherwise.
         */
        [[nodiscard]] iterator find(const entity_type entt) const noexcept { return view ? view->find(entt) : iterator{}; }
         ...
         */
        [[nodiscard]] decltype(auto) operator[](const entity_type entt) const { return std::get<0>(pools)->get(entt); }

        /**
         * @brief Checks if a view is fully initialized.
         * @return True if the view is fully initialized, false otherwise.
         */
        [[nodiscard]] explicit operator bool() const noexcept { return (std::get<0>(pools) != nullptr); }
         ...
         * @return True if the view contains the given entity, false otherwise.
         */
        [[nodiscard]] bool contains(const entity_type entt) const noexcept { return view && view->contains(entt); }
         ...
         * @return An iterable object to use to _visit_ the view.
         */
        [[nodiscard]] iterable each() const noexcept { return view ? std::get<0>(pools)->each() : iterable{}; }
        ...
        [[nodiscard]] auto operator|(const basic_view<get_t<OGet...>, exclude_t<OExclude...>>& other) const noexcept
        {
            return internal::view_pack(std::tuple_cat(pools, other.pools),
