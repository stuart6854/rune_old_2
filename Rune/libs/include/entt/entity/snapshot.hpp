...
namespace entt
{
    ...
    public:
         ...
         */
        basic_snapshot(const registry_type& source) noexcept : reg{ &source } {}
        ...
        const basic_snapshot& get(Archive& archive, It first, It last, const id_type id = type_hash<Type>::value()) const
        {
            ...
            if (const auto* storage = reg->template storage<Type>(id); storage && !storage->empty())
            {
                ...
                for (; first != last; ++first)
                {
                    ...
                    {
                        archive(entt);
                        std::apply([&archive](auto&&... args) { (archive(std::forward<decltype(args)>(args)), ...); },
                                   storage->get_as_tuple(entt));
                    }
                    else
                    {
        ...
        [[deprecated("use .get<Type>(archive, first, last) instead")]] const basic_snapshot& component(Archive& archive,
                                                                                                       It first,
                                                                                                       It last) const
        {
    ...
    public:
         ...
         * @param source A valid reference to a registry.
         */
        basic_snapshot_loader(registry_type& source) noexcept : reg{ &source }
        {
    ...
    public:
         ...
         * @param source A valid reference to a registry.
         */
        basic_continuous_loader(registry_type& source) noexcept : remloc{ source.get_allocator() }, reg{ &source } {}
        ...
        [[deprecated("use .component<Type>(archive, members...) instead")]] basic_continuous_loader& component(Archive& archive,
                                                                                                               Member Clazz::*... member)
        {
            (
                [&](auto& storage)
         ...
         * @return A non-const reference to this loader.
         */
        [[deprecated("use .get<Entity>(archive) instead")]] basic_continuous_loader& shrink() { return *this; }
