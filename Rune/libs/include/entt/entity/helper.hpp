...
namespace entt
{
    ...
    class as_view
    {
        ...
        {
            return reg.template view<constness_as_t<typename Get::value_type, Get>...>(
                exclude_t<constness_as_t<typename Exclude::value_type, Exclude>...>{});
        }
    ...
    public:
         ...
         */
        as_view(registry_type& source) noexcept : reg{ source } {}
    ...
    class as_group
    {
        ...
        auto dispatch(owned_t<Owned...>, get_t<Get...>, exclude_t<Exclude...>) const
        {
            ...
            {
                return reg.template group_if_exists<typename Owned::value_type...>(get_t<typename Get::value_type...>{},
                                                                                   exclude_t<typename Exclude::value_type...>{});
            }
            else
            {
                return reg.template group<constness_as_t<typename Owned::value_type, Owned>...>(
                    get_t<constness_as_t<typename Get::value_type, Get>...>{},
                    exclude_t<constness_as_t<typename Exclude::value_type, Exclude>...>{});
            }
        }
    ...
    public:
         ...
         * @param source A valid reference to a registry.
         */
        as_group(registry_type& source) noexcept : reg{ source } {}
    ...
    typename Registry::entity_type to_entity(const Registry& reg, const Component& instance)
    {
        if (const auto* storage = reg.template storage<Component>(); storage)
        {
            ...
            {
                if (const auto dist = (addr - std::addressof(storage->get(*it)));
                    dist >= 0 && dist < static_cast<decltype(dist)>(page_size))
                {
                    return *(it + dist);
                }
            }
    ...
    struct sigh_helper<Registry>
    {
         ...
         */
        sigh_helper(registry_type& ref) : bucket{ &ref } {}
         ...
         */
        [[nodiscard]] registry_type& registry() noexcept { return *bucket; }
    ...
    struct sigh_helper<Registry, Type> final : sigh_helper<Registry>
    {
         ...
         * @param id Optional name for the underlying storage to use.
         */
        sigh_helper(registry_type& ref, const id_type id = type_hash<Type>::value()) : sigh_helper<Registry>{ ref }, name{ id } {}
