...
namespace entt
{
    ...
    class basic_scheduler
    {
        ...
        struct continuation
        {
            continuation(process_handler* ref) noexcept : handler{ ref } {}
            ...
            continuation then(Args&&... args)
            {
                ...
                auto proc =
                    typename process_handler::instance_type{ new Proc{ std::forward<Args>(args)... }, &basic_scheduler::deleter<Proc> };
                handler->next.reset(
                    new process_handler{ std::move(proc), &basic_scheduler::update<Proc>, &basic_scheduler::abort<Proc>, nullptr });
    ...
    public:
         ...
         */
        [[nodiscard]] size_type size() const noexcept { return handlers.size(); }

        /**
         * @brief Returns true if at least a process is currently scheduled.
         * @return True if there are scheduled processes, false otherwise.
         */
        [[nodiscard]] bool empty() const noexcept { return handlers.empty(); }
        ...
        auto attach(Args&&... args)
        {
            ...
            auto&& ref = handlers.emplace_back(
                process_handler{ std::move(proc), &basic_scheduler::update<Proc>, &basic_scheduler::abort<Proc>, nullptr });
            // forces the process to exit the uninitialized state
