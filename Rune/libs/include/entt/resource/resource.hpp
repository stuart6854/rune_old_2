...
namespace entt
{
    ...
    public:
         ...
         */
        template <typename Other>
        resource(const resource<Other>& other, element_type& res) noexcept : value{ other.value, std::addressof(res) }
        {
        }
        ...
        resource(const resource<Other>& other) noexcept : value{ other.value }
        {
        }
         ...
         */
        template <typename Other, typename = std::enable_if_t<is_acceptable_v<Other>>>
        resource(resource<Other>&& other) noexcept : value{ std::move(other.value) }
        {
        }
         ...
         */
        [[nodiscard]] element_type& operator*() const noexcept { return *value; }

        /*! @copydoc operator* */
        [[nodiscard]] operator element_type&() const noexcept { return *value; }

        /**
         * @brief Returns a pointer to the managed resource.
         * @return A pointer to the managed resource.
         */
        [[nodiscard]] element_type* operator->() const noexcept { return value.get(); }
         ...
         */
        [[nodiscard]] const handle_type& handle() const noexcept { return value; }
